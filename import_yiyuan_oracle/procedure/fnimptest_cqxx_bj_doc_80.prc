create or replace procedure fnimptest_cqxx_bj_doc_80(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
  3-----------2700  商品房
  79----------2483  个人建房
  80----------2484  单位建房
...
begin
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eisdoc_yy' and a.typeid=l_typeid 
                order by a.keycode
             /*select keycode from t_keycode_yy where keycode='200905070023'*/)loop
	l_Cqzh := null;
     ...
     if img < 0 then
       ...
       if iCount=0 and iCount2=0 then
         ...
         insert into SHD (recid,SLR,Slr_Rq,SLYJ,CSR,Csr_Rq,Csyj,Fsr,Fsr_Rq,FSYJ,DBR,DBYJ,DBR_RQ)
           ...
           select count(1) into iCount4 from eisdoc_yy.owner s where s.keycode=rs.keycode;
		  ...
		  b.h_use 房屋用途,
            ...
            elsif l_stepno in (6,7,8,9,10) then
