create or replace procedure fnimptest_ysxx_bj_doc  is
/****************************************************************************************
  31-----------2538  预售许可登记
...
begin
  for rs in(select a.keycode,p.declareco from t_keycode_yy a,eisdoc_yy.presell p
              where a.keycode=p.keycode
                and a.typeid = 31
                and a.keycode not in ('200904230002')
                 and a.t_user='eisdoc_yy')loop
    ...
    if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into YSXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,T_USER)values(iRecid,rs.keycode,v_msg,31,1,'eisdoc_yy');
    else
        insert into YSXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,T_USER)values(iRecid,rs.keycode,v_msg,31,1,'eisdoc_yy');
        --插入YSXX
        insert into YSXX(RECID,SFDB,YSDUIXIANG,YSXKZH,YSQX,SLBH,YWMC,XKZLX)
            select iRecid,0,'群众',p.certcode,'1',rs.keycode,'预售许可登记','商品'
                from eisdoc_yy.presell p
            where p.keycode=rs.keycode;
        --插入YSLPXX
        insert into YSLPXX(RECID, LPBH,y_Qh,y_zh)
           select iRecid,b.buildcode,b.land_no,b.build_no from eisdoc_yy.presell_build b where b.keycode=rs.keycode;
        --插入XMXX
        insert into XMXX(RECID,GSLXDH,DWDZ,FDDBR,XMBH,XMMC,XMDZ,ZJHM,
          ...
          select iRecid,d.developer_tel,d.developer_add,d.developer_man,p.projectno,p.itemname,p.address,d.developer_register,
            ...
            from eistran_yy.developer d, eisdoc_yy.presell p
