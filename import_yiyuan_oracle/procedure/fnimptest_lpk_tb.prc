create or replace procedure fnimptest_lpk_tb(ibizid integer) is
/****************************************************************************************
...
begin
  for rs in(select b.keycode, a.ITEMNAME ITEMNAME,b.buildcode buildcode,decode(b.buildname, NUll, a.itemname, b.buildname) buildname,c.DEVELOPER_NAME DEVELOPER_NAME,
                   ...
                   b.BUILD_NO BUILD_NO,a.CERTCODE CERTCODE,a.ALLAREA ALLAREA,
		   b.cellnum CELLNUM,b.floorall FLOORALL,b.FLOOR_UNDER FLOOR_UNDER,b.floorall - b.FLOOR_UNDER FLOOR_UP,b.setnum SETNUM
               from eisdoc_yy.Presell a, eisdoc_yy.PRESELL_BUILD b,eistran_yy.Developer c
             where a.keycode=b.keycode  and a.declareco=c.code and a.keycode not in ('200904230002')
              and b.buildcode not in (select q.pre_buildcode from LPK_LOG q))loop
    ...
    if img < 0 then
       ...
       insert into tbbuilding(lpbh,lpmc,xmmc,qymc,zl,yt,jg,qh,zh,recid,cllx,ssq,cjh,th,lpzt,ysxkzh,mj,jgxg,salestatus,sfjg,dys,zcs,dscs,dxcs,ts)
          values(rs.buildcode,rs.buildname,rs.ITEMNAME,rs.developer_name,rs.address,rs.houseuse,rs.frame,rs.land_no,rs.build_no,iRecid,
