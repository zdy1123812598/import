create or replace procedure fnimptest_zydj_doc_next(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eisdoc_yy'
	...
	select s.ocertno into l_Cqzh from eisdoc_yy.person s where s.keycode=rs.keycode;
	--插入房屋明细（FWMX）
       insert into fwmx(uuid,recid,createdate,fwbh,dymj,syqzh,status,fwzk_uuid)
         ...
         --插入房屋状况（FWZK）
         select count(1) into iCount5 from eisdoc_yy.history_room b where b.keycode=rs.keycode;
         if iCount5>0 then	   
		...
		b.h_use 房屋用途,
            ...
            select count(1) into iCount6 from eisdoc_yy.room b where b.roomcode in (select a.roomcode from eisdoc_yy.owner a where a.keycode=rs.keycode);
            if iCount6 >0 then             
              for rn in (select h.roomcode from eisdoc_yy.owner h where h.keycode=rs.keycode)loop
                insert into fwzk (uuid,recid,createdate,syqzh,FWBH,QH,ZH,FWMC,FH,JZMJ,TNMJ,GTMJ,SZCS,YTDL,FWYT,FWLX,FWZL,SSQ,th,cjh,status)
                 select
                    sys_guid(),
                    iRecid,
		    ...
		    b.h_use 房屋用途,
              ...
              update zydj_log set sffwxx=1 where recid=iRecid;
            end if; 
          end if;
          --插入共有信息
          ...
          if l_stepno=10 or iCount1>0  then
