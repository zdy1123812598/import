create or replace procedure fnimptest_ygspfdyygdj_doc(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  ...
  for rs in(
     select a.keycode,a.appman,a.appsex,a.appnation,a.appbirth,a.appaddress,a.appidcno,a.appcellphone,a.yshtno
      from t_keycode_yy s ,eisdoc_yy.operation a where s.keycode=a.keycode and s.typeid=l_typeid and s.t_user='eisdoc_yy'
       order by s.keycode
    )loop
     ...
     if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into SPFDYYG_LOG(RECID, KEYCODE, MSG, Type_Id,Isgd,t_User)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eisdoc_yy');
     else
       insert into SPFDYYG_LOG(RECID, KEYCODE,  MSG, Type_Id,Isgd,t_User)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eisdoc_yy');
       --插入抵押信息(DYXX)
       select count(1) into iCount from eisdoc_yy.predeclare a where a.keycode=rs.keycode;
       if iCount=0 then
         update SPFDYYG_LOG set sjyy='无数据' where recid=iRecid;
           ...
           insert into dyxx (recid,ywmc,slbh,dyqygdjzmh,TXQR,txqrzjmc,txqrzjhm,txqrlxdh,txqrlxr,txqrfrdb,syqr,syqrzjmc,
              ...
              select iRecid,l_ywmc,rs.keycode,c.code,a.m_name, a.m_type, a.m_code, a.m_tel, a.m_man, a.m_lawman,
                ...
                from eisdoc_yy.PREMM_PERSON a, eisdoc_yy.PREMO_PERSON b,eisdoc_yy.predeclare c,
                     eisdoc_yy.PREMORTAGAGE d
