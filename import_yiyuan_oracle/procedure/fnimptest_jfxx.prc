create or replace procedure fnimptest_jfxx is
/****************************************************************************************
  36-----------2698  查封
  140----------2698  预查封
  97-----------2699  解封
...
begin
  for rs in(
    select a.keycode
     ...
     where a.workcode in (select workcode from eistran_yy.WorkFlow where sercode=97 and stepno in (0,1,2,3,4,5,6 ))
      and a.type_id = 97  order by a.keycode
    )loop
    ...
    if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into CFXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,LX)values(iRecid,rs.keycode,v_msg,97,0,'解封');
     else
       insert into CFXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,LX)values(iRecid,rs.keycode,v_msg,97,0,'解封');
       --插入CFDJ
