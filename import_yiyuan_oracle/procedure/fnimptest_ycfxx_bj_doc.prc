create or replace procedure fnimptest_ycfxx_bj_doc is
/****************************************************************************************
  36-----------2698  查封
  140----------2698  预查封
  97-----------2699  解封
...
begin
  ...
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eisdoc_yy'
    ...
    if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into YCFXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,lx,T_USER)values(iRecid,rs.keycode,v_msg,140,1,'预查封','eisdoc_yy');
     else
       insert into YCFXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,lx,T_USER)values(iRecid,rs.keycode,v_msg,140,1,'预查封','eisdoc_yy');
