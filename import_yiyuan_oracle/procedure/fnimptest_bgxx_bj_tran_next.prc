create or replace procedure fnimptest_bgxx_bj_tran_next (l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  ...
  for rs in(select s.keycode from t_keycode_yy s where  s.t_user = 'eistran_yy' and s.TYPEID=l_typeid
               order by s.keycode
            /*select s.keycode from t_keycode_yy s where s.keycode in ('201211060029','201211060027')*/)loop
     l_Cqzh := null;
     ...
     if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into BGDJ_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,t_user)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
     else
       insert into BGDJ_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,t_user)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
       --插入cqxx
       ...
       if iCount1=0 then
         ...
         insert into fwzk (uuid,recid,createdate,syqzh,FWBH,QH,ZH,FWMC,FH,JZMJ,TNMJ,GTMJ,SZCS,YTDL,FWYT,FWLX,FWZL,SSQ,th,cjh,status)
          ...
          if l_stepno=10 or iCount2>0  then
