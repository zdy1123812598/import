create or replace procedure fnimptest_dyxx_bj_tran(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  ...
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eistran_yy' 
    ...
    /*select keycode from t_keycode_yy where keycode='201301060013'*/)loop
    ...
    if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into dyXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,T_USER)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
     else
       insert into dyXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,T_USER)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
       --插入dyxx
       select count(1) into iCount1 from eistran_yy.MM_Person a where a.keycode=rs.keycode;
       if iCount1=0 then
