create or replace procedure fnimptest_ygspfygdj_tran(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  iActdefid := 1065;
  iROLEID := 9629;
  for rs in(select a.keycode,b.yshtno from t_keycode_yy a,eistran_yy.operation b where a.typeid=l_typeid and a.T_USER='eistran_yy'
     ...
     if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into SPFYG_LOG(RECID, KEYCODE, MSG, Type_Id,Isgd,T_USER)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
     else
       insert into SPFYG_LOG(RECID, KEYCODE,  MSG, Type_Id,Isgd,T_USER)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
       select count(1) into iCount4 from eistran_yy.predeclare a where a.keycode=rs.keycode;
       if iCount4=0 then
         ...
         insert into cqxx (recid,slbh,ywmc,ygzh,cqr,zjhm,lxdh,foreshow,sfdb,status)
            select iRecid,
              ...
              from eistran_yy.predeclare a,eistran_yy.operation b
         ...
         if l_typeid<>100 then
          ...
          select count(1) into iCount5 from eistran_yy.operation a where a.keycode=rs.keycode;
          if iCount5=0 then
