create or replace procedure fnimptest_zydj_tran(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  ...
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eistran_yy' 
             ...
             /*select s.keycode from t_keycode_yy s where s.keycode in ('201403050056','201210080018')*/)loop
	 l_Cqzh := null;
     ...
     if img < 0 then
       ...
       if iCount2=0 then
         update ZYDJ_LOG set sjyy='无数据' where recid=iRecid;
          ...
          if l_typeid<>115 then
           --插入房屋状况（FWZK）
           select a.keycode into l_Cqzh from eistran_yy.person a where a.keycode=rs.keycode;
		...
		b.h_use 房屋用途,
             ...
             select a.keycode into l_Cqzh from eistran_yy.person a where a.keycode=rs.keycode and a.subkeycode=2;
             insert into fwzk (uuid,recid,createdate,syqzh,FWBH,QH,ZH,FWMC,FH,JZMJ,TNMJ,GTMJ,SZCS,YTDL,FWYT,FWLX,FWZL,SSQ,th,cjh,status)
             select
                sys_guid(),
                iRecid,
