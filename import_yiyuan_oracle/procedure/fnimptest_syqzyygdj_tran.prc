create or replace procedure fnimptest_syqzyygdj_tran(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  iActdefid := 1048;
  iROLEID := 9611;
  for rs in(select a.keycode from t_keycode_yy a  where a.typeid=l_typeid and a.T_USER='eistran_yy'
     ...
     if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into SYQZYYG_LOG(RECID, KEYCODE, MSG, Type_Id,Isgd,T_USER)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
     else
       insert into SYQZYYG_LOG(RECID, KEYCODE,  MSG, Type_Id,Isgd,T_USER)values(iRecid,rs.keycode,v_msg,l_typeid,1,'eistran_yy');
       --插入产权信息(CQXX)
       select count(1) into iCount1 from eistran_yy.PREDECLARE where keycode=rs.keycode;
       if iCount1=0 then
