create or replace procedure fnimptest_cqxx_bj_tran_80(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
  3-----------2700  商品房
  79----------2483  个人建房
  80----------2484  单位建房
...
begin
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eistran_yy' 
    ...
    if img < 0 then
       ...
       --插入房屋状况（FWZK）
       select a.ocertno into l_Cqzh from eistran_yy.person a where a.keycode=rs.keycode;
       insert into fwzk (uuid,recid,createdate,syqzh,FWBH,QH,ZH,FWMC,FH,JZMJ,TNMJ,GTMJ,SZCS,YTDL,FWYT,FWLX,FWZL,SSQ,th,cjh,status)
	    ...
	    b.h_use 房屋用途,
            ...
            /*,
            b.curcell ,
            b.hvalue 房屋总价*/ from eistran_yy.Room b
