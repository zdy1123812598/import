create or replace procedure fnimptest_insertroom_wysxk(p_recid integer,p_LANDNO varchar2,p_BUILDNO varchar2) is
/****************************************************************************************
...
begin
  ...
  insert into tbroom(uuid,recid,createdate,roomnum,szcs,fwzt,unitnum,
     ...
     select sys_guid(),p_recid,sysdate,row_number() over(partition by CURFLOOR, CURCELL order by ROOM_NO),a.curfloor,3,regexp_replace(a.curcell,'[^0-9]'),
            a.curfloor,a.h_use,lpad(regexp_replace(a.curcell,'[^0-9]'),2,0)||lpad(a.curfloor,2,0)||lpad(regexp_replace(a.curcell,'[^0-9]'),2,0),
            regexp_replace(a.curcell,'[^0-9]'),a.curfloor,a.house_sit/*lpad(regexp_replace(a.curcell,'[^0-9]'),2,0)||lpad(a.curfloor,2,0)||lpad(regexp_replace(a.curcell,'[^0-9]'),2,0)||'∫≈'*/,l_zh,l_qh,
       ...
       from /*eisdoc_yy.room*/eisdoc_wys_room a where a.land_no=p_LANDNO and a.build_no=p_BUILDNO and a.curfloor>'0';
  insert into tbroom(uuid,recid,createdate,roomnum,szcs,fwzt,
     ...
     select sys_guid(),p_recid,sysdate,row_number() over(partition by CURFLOOR, CURCELL order by ROOM_NO),a.curfloor,3,
            a.curfloor,'¥¢≤ÿ “'||regexp_replace(a.room_no,'[^0-9]'),a.curfloor,a.house_sit,l_zh,l_qh,
