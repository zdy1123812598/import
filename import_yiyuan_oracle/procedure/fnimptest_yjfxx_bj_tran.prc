create or replace procedure fnimptest_yjfxx_bj_tran is
/****************************************************************************************
  36-----------2698  查封
  140----------2698  预查封
  97-----------2699  解封
...
begin
  ...
  /*if iRet=0 then
    ...
    if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into YCFXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,lx,t_User)values(iRecid,rs.keycode,v_msg,141,1,'预解封','eistran_yy');
     else
       insert into YCFXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,lx,t_User)values(iRecid,rs.keycode,v_msg,141,1,'预解封','eistran_yy');
       --插入CFDJ
