create or replace procedure fnimptest_bgxx_bj_tran (l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
...
begin
  ...
  for rs in(select s.keycode from t_keycode_yy s where  s.t_user = 'eistran_yy' and s.TYPEID=l_typeid
               order by s.keycode
              /*select s.keycode from t_keycode_yy s where s.keycode='201110210078'*/)loop
      l_Cqzh := null;
