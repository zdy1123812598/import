create or replace procedure fnimptest_zxxx_tran is
/****************************************************************************************
...
begin
  ...
  for rs in(select a.keycode from t_keycode_yy a where a.typeid = 26 
              and a.t_user='eistran_yy'  order by a.keycode)loop
    select s.humanid into l_humancode from tbhuman s where s.humanname=(select a.accman from eistran_yy.operation a where a.keycode=rs.keycode) and s.cantoncode=10;
     ...
     if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into zxxx_log(RECID, KEYCODE, MSG, Type_Id,Isgd,t_user)values(iRecid,rs.keycode,v_msg,26,1,'eistran_yy');
     else
       insert into zxxx_log(RECID, KEYCODE,  MSG, Type_Id,Isgd,t_user)values(iRecid,rs.keycode,v_msg,26,1,'eistran_yy');
