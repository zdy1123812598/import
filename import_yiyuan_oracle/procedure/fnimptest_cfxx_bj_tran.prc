create or replace procedure fnimptest_cfxx_bj_tran is
/****************************************************************************************
  36-----------2698  查封
  140----------2698  预查封
  97-----------2699  解封
...
begin
  ...
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eistran_yy' 
             and a.typeid=36  order by a.keycode
             /*select a.keycode from t_keycode_yy a where a.keycode='201203270046'*/)loop
    ...
    if img < 0 then
       v_msg := '收件失败，原因：'||imsg;
       insert into CFXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,LX,T_USER)values(iRecid,rs.keycode,v_msg,36,1,'查封','eistran_yy');
     else
       insert into CFXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,LX,T_USER)values(iRecid,rs.keycode,v_msg,36,1,'查封','eistran_yy');
       --插入CFDJ
       select count(1) into iCount1 from eistran_yy.Freeze a where a.keycode=rs.keycode;
       if iCount1=0 then
         ...
         insert into SFHJ(RECID,SFHJ)
          ...
          select count(1) into iCount3 from eistran_yy.operation a where a.keycode=rs.keycode and a.accman like '%录档%';
