create or replace procedure fnhetonginfo(l_bargain_no varchar2,p_recid integer) is
  v_clob CLOB ;
...
begin
   v_count := 1;
   suf_count :=1;
    select a.bargain_no,a.text ,a.buyman into l_no,v_clob,l_buyman from eisweb_yy.bargain_save a where a.bargain_no=l_bargain_no;
    delete from tbhetongcaoben where t_no=l_bargain_no;
    delete from HTGYRXX where recid= p_recid;
    WHILE v_char IS NOT NULL LOOP
      v_char := regexp_substr(v_clob, '[^|]{1,}', 1, v_cnt);
      v_char := TRIM(v_char);
      v_cnt  := v_cnt + 1;

      IF v_char IS NOT NULL THEN
        v_len2 :=LENGTH(v_char);
        v_loc :=instr(v_char,'=');
        l_name :=substr( v_char,0,v_loc-1);
        l_value :=substr( v_char,v_loc+1,v_len2-v_loc);
        INSERT INTO tbhetongcaoben(t_no, t_name,t_value) VALUES (l_no ,l_name, l_value);
      END IF;
    END LOOP;
