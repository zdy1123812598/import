create or replace procedure fnimptest_cqxx_80(l_typeid integer,ibizid integer,l_ywmc varchar2) is
/****************************************************************************************
  3-----------2700  商品房
  79----------2483  个人建房
  80----------2484  单位建房
...
begin
  ...
  /*if iRet=0 then
    ...
    if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into CQXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD)values(iRecid,rs.keycode,v_msg,l_typeid,0);
     else
       insert into CQXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD)values(iRecid,rs.keycode,v_msg,l_typeid,0);
       --插入cqxx
       ...
       insert into fwzk (uuid,recid,createdate,syqzh,FWBH,QH,ZH,FWMC,FH,JZMJ,TNMJ,GTMJ,SZCS,YTDL,FWYT,FWLX,FWZL,SSQ,th,cjh,status)
	    ...
	    b.h_use 房屋用途,
            ...
            /*,
            b.curcell ,
            b.hvalue 房屋总价*/ from eistran_yy.person a,eistran_yy.Room b
         where a.keycode=b.keycode
           and a.keycode=rs.keycode;
        --插入共有信息
