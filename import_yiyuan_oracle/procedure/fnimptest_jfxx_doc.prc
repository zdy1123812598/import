create or replace procedure fnimptest_jfxx_doc is
/****************************************************************************************
  36-----------2698  查封
  140----------2698  预查封
  97-----------2699  解封
...
begin
  ...
  for rs in(select a.keycode from t_keycode_yy a where a.t_user='eisdoc_yy'
     ...
     if img < 0 then
        v_msg := '收件失败，原因：'||imsg;
        insert into CFXX_LOG(RECID, KEYCODE, MSG, Type_Id,ISGD,LX,t_User)values(iRecid,rs.keycode,v_msg,97,1,'解封','eisdoc_yy');
     else
       insert into CFXX_LOG(RECID, KEYCODE,  MSG, Type_Id,ISGD,LX,t_User)values(iRecid,rs.keycode,v_msg,97,1,'解封','eisdoc_yy');
       --插入CFDJ
       select a.recnum into l_recnum from tbrec a where a.recid=iRecid;
       select count(1) into iCount  from eisdoc_yy.Freeze a where a.keycode=rs.keycode;
       select count(1) into iCount4 from eisdoc_yy.freeze a where a.logoutkeycode=rs.keycode;
       if iCount=0 and iCount4=0 then
         update CFXX_LOG set sjyy='无数据' where  recid=iRecid;
          ...
          --插入房屋明细（FWMX）
          for rn in (select * from eisdoc_yy.freeze s where s.keycode=rs.keycode)loop
            insert into fwmx(uuid,recid,createdate,fwbh,dymj,syqzh,status,cqr,jfbh,jfrq,jfr)
             select sys_guid(),iRecid,sysdate,a.roomcode,a.b_area,rn.certcode,2,rn.owner,rn.logoutkeycode,rn.f_date,rn.f_man
               from eisdoc_yy.room a
             where a.roomcode = rn.roomcode and rownum=1;
          end loop;
       elsif iCount=0 and iCount4>0 then
          insert into CFDJ(RECID,SLBH,STATUS,CFBZ,CFBH,CFDW,CFDX,CHAFENLX,CFRQ,CQZH,YWMC,Jfbh)
            select iRecid,
                   rs.keycode,
                   0,
                   ...
                   a.logoutkeycode
              from eisdoc_yy.Freeze a
             where a.logoutkeycode = rs.keycode and rownum=1;
          --插入房屋明细（FWMX）
          for rn in (select * from eisdoc_yy.freeze s where s.logoutkeycode=rs.keycode)loop
            insert into fwmx(uuid,recid,createdate,fwbh,dymj,syqzh,status,cqr,jfbh,jfrq,jfr)
             select sys_guid(),iRecid,sysdate,a.roomcode,a.b_area,rn.certcode,2,rn.owner,rn.logoutkeycode,rn.f_date,rn.f_man 
               from eisdoc_yy.room a
             where a.roomcode = rn.roomcode and rownum=1;
       ...
       select count(1) into iCount3 from eisdoc_yy.operation a where a.keycode=rs.keycode and a.accman like '%录档%';
       select count(1) into iCount2 from eisdoc_yy.operation a where a.keycode=rs.keycode;
        if iCount2>0 then
       ...
       if l_stepno=11 or iCount3>0 then
